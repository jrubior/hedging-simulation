...
function results = fit_svx_t_pf_full(r, m, S, varargin)
%FIT_SVX_T_PF_FULL  Conditional CAPM + SV-X with Student-t errors (monthly).
%
% Model (Section 1):
%   r_t = alpha + beta*m_t + gamma'*s_t + exp(0.5*h_t)*eps_t
%   eps_t ~ t_nu(0,1)   (variance normalized to 1)
%   h_t = omega + phi*(h_{t-1}-omega) + delta'*s_t + sigma_h*xi_t
%   xi_t ~ N(0,1)
%
% Estimation:
%   - Bootstrap particle filter for log-likelihood (fixed seed for smoothness)
%   - MLE via fminunc
%
% Outputs:
%   - Parameter estimates + (optional) standard errors via numerical Hessian
%   - Reported performance statistics (annualized return/vol/downside vol/beta/alpha/sortino)
%   - Filtered log-vol summary path (posterior mean/var from PF weights)
%
% Usage:
%   results = fit_svx_t_pf_full(r, m, S);
%
% Optional name-value pairs:
%   'NParticles'      (default 2000)
%   'Seed'            (default 123)
%   'ComputeStdErr'   (default true)
%   'HessStep'        (default 1e-4)
%   'Display'         (default 'iter')  % fminunc display
%   'MaxIter'         (default 200)
%   'ResampleThresh'  (default 0.5)
%
% Notes:
%   - r and m are monthly excess returns (over cash).
%   - For gross returns, transform externally before calling.
%   - With PF likelihoods, numerical Hessians can be noisy; keep Seed fixed.
%
% ------------------------------------------------------------
    ...
    % -------- Parse options --------
    pOpt = inputParser;
    pOpt.addParameter('NParticles', 2000);
    ...
    omega0  = log(v0);   % log-variance level
    phi0    = 0.90;
    sigmah0 = 0.20;
    delta0  = zeros(K,1);
    nu0     = 8;

    theta0 = pack_params(alpha0, beta0, gamma0, omega0, phi0, sigmah0, delta0, nu0);
    ...
    if opt.ComputeStdErr
        ...
        % Invert Hessian (observed information)
...
function stats = compute_report_stats(r, m, Sz, p, pf_out)
    ...
    % Fitted conditional mean and volatility proxy (PF posterior mean of h_t)
    mu_hat = p.alpha + p.beta*m + (K>0)*(Sz*p.gamma);
    h_hat  = pf_out.h_mean;                 % T x 1
    sig_hat = exp(0.5*h_hat);               % T x 1

    % Standardized residuals
    eps_hat = (r - mu_hat) ./ sig_hat;

    % Annualized return/vol (from realized data)
    mean_m = mean(r,'omitnan');
    vol_m  = std(r,0,'omitnan');
    ann_ret = 12*mean_m;
    ann_vol = sqrt(12)*vol_m;

    % Downside volatility: std of r conditional on r<0, annualized
    neg = (r < 0) & isfinite(r);
    if any(neg)
        dvol_m = std(r(neg),0,'omitnan');
        ann_dvol = sqrt(12)*dvol_m;
        sortino = (12*mean_m) / (sqrt(12)*dvol_m); % = sqrt(12)*mean/ downside std
    else
        ann_dvol = NaN;
        sortino  = NaN;
    end

    % CAPM regression beta/alpha (sample OLS against market)
    Xcapm = [ones(T,1) m];
    bcapm = Xcapm \ r;
    alpha_ols_m = bcapm(1);
    beta_ols    = bcapm(2);

    % Jensen alpha: model alpha (monthly) and annualized
    alpha_model_m = p.alpha;
    alpha_model_ann = 12*p.alpha;

    % Beta: model beta (constant) and OLS beta
    beta_model = p.beta;

    % Skewness/kurtosis (sample, not percent)
    rv = r(isfinite(r));
    ...
    % Convenience: volatility of fitted innovations (annualized)
...
function [nll, out] = negloglik_pf(theta, r, m, Sz, pf)
    ...
    for t = 1:T
        ...
        logw = log_t_density_var1(e, nu) - log(sigma);
...
%% ============================================================
%  Student-t log density for eps with Var(eps)=1
%% ============================================================
function logf = log_t_density_var1(eps, nu)
    s = sqrt((nu-2)/nu);
    z = eps ./ s;

    logc = gammaln((nu+1)/2) - gammaln(nu/2) - 0.5*log(nu*pi);
    logfz = logc - ((nu+1)/2) .* log(1 + (z.^2)/nu);
...
%% ============================================================
function theta = pack_params(alpha, beta, gamma, omega, phi, sigmah, delta, nu)
    eta_phi = atanh(max(min(phi,0.999),-0.999));
    eta_sh  = log(max(sigmah, 1e-6));
    eta_nu  = log(max(nu-2, 1e-6));
    theta = [alpha; beta; gamma(:); omega; eta_phi; eta_sh; delta(:); eta_nu];
end
