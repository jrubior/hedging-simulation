...
function results = fit_svx_jumps_pf_full(r, m, S, varargin)
%FIT_SVX_JUMPS_PF_FULL  SV-X with Normal + state-dependent Exponential jumps.
%
% Model:
%   r_t = alpha + beta*m_t + gamma'*s_t + exp(0.5*h_t)*eps_t + J_t*Z_t
%   eps_t ~ N(0,1)
%   J_t   ~ Bernoulli(p_{j,t})     [state-dependent jump indicator]
%   Z_t   ~ Exp(1/mu_j)            [jump size, mean mu_j > 0]
%   h_t = omega + phi*(h_{t-1}-omega) + delta'*s_t + sigma_h*xi_t
%   xi_t  ~ N(0,1)
%
% Jump intensity linked to states:
%   p_{j,t} = sigmoid(psi_0 + psi'*s_t)
%
% The observation density is a closed-form mixture:
%   p(r_t|h_t) = (1 - p_{j,t}) * Normal(r_t; mu_t, sigma_t^2)
%              +  p_{j,t}       * EMG(r_t; mu_t, sigma_t, mu_j)
%
% Parameters:
%   Mean eq:  alpha, beta, gamma (K)
%   Vol eq:   omega, phi, sigma_h, delta (K)
%   Jump eq:  psi_0, psi (K), mu_j          <-- state-dependent intensity
%
...
% ------------------------------------------------------------
    ...
    sigmah0  = 0.20;
    delta0   = zeros(K,1);
    psi00    = log(0.05/0.95);   % logit(0.05) ≈ -2.944
    psi0     = zeros(K,1);       % no state dependence initially
...
function print_report(res)
    ...
    fprintf('\n======= SV-X (Normal + State-Dependent Jump) REPORT =======\n');
    ...
    fprintf('\n--- Jump intensity: p_{j,t} = sigmoid(psi_0 + psi''*s_t) ---\n');
...
%  (Normal innovations + state-dependent Exponential jumps)
%  Observation density:
%    p(r_t|h_t) = (1-p_{j,t})*Normal + p_{j,t}*EMG
%  where p_{j,t} = sigmoid(psi_0 + psi'*s_t)
%% ============================================================
function [nll, out] = negloglik_pf(theta, r, m, Sz, pf)
    ...
    % Precompute time-varying jump probabilities (depend on observed s_t only)
    pj_path = 1 ./ (1 + exp(-(psi0 + (K>0)*(Sz*psi))));  % T x 1
...
function theta = pack_params(alpha, beta, gamma, omega, phi, sigmah, ...
    ...
    % psi0, psi are unconstrained (sigmoid handles (0,1))
...
function p = unpack_params(theta, K)
    ...
    if K > 0
        p.psi = theta(idx+1:idx+K); idx = idx + K;
    else
        p.psi = zeros(0,1);
    end
