...
function results = fit_svx_jump_pf_full(r, m, S, varargin)
%FIT_SVX_JUMP_PF_FULL  SV-X with Normal innovations + Exponential jumps (monthly).
%
% Model:
%   r_t = alpha + beta*m_t + gamma'*s_t + exp(0.5*h_t)*eps_t + J_t*Z_t
%   eps_t ~ N(0,1)
%   J_t   ~ Bernoulli(p_j)          [jump indicator]
%   Z_t   ~ Exp(1/mu_j)             [jump size, mean mu_j > 0]
%   h_t = omega + phi*(h_{t-1}-omega) + delta'*s_t + sigma_h*xi_t
%   xi_t  ~ N(0,1)
%
% The observation density is a closed-form mixture:
%   p(r_t|h_t) = (1-p_j) * Normal(r_t; mu_t, sigma_t^2)
%              +  p_j    * EMG(r_t; mu_t, sigma_t, mu_j)
%
% where EMG = Exponentially Modified Gaussian (Normal + Exponential convolution).
%
% Key features:
%   - Exponential jumps generate positive skewness and excess kurtosis
%   - Closed-form density: no numerical integration needed
%   - SV provides time-varying volatility (unconditional fat tails)
%
% Usage:
%   results = fit_svx_jump_pf_full(r, m, S);
%
...
%  (Normal innovations + Exponential jumps)
%  Observation density is a closed-form mixture:
%    p(r|h) = (1-p_j)*Normal + p_j*EMG
%% ============================================================
function [nll, out] = negloglik_pf(theta, r, m, Sz, pf)
    ...
    lam = 1/mu_j;   % exponential rate parameter

    % Precompute log constants
    log_1mpj    = log(max(1 - p_j, 1e-15));
    log_pj      = log(max(p_j, 1e-15));
    log_lam     = log(lam);
    ...
    for t = 1:T
        ...
        sigma = exp(0.5*h);        % N x 1
        e = (r(t) - mu(t)) ./ sigma;

        % --- No-jump component: Normal ---
        log_f0 = -half_log2pi - log(sigma) - 0.5*e.^2;

        % --- Jump component: EMG (Normal * Exponential convolution) ---
        % f_EMG(r) = lam * exp(lam*(mu-r) + 0.5*lam^2*sigma^2) * Phi(x)
        % where x = (r - mu - lam*sigma^2) / sigma = e - lam*sigma
...
%% ============================================================
function theta = pack_params(alpha, beta, gamma, omega, phi, sigmah, delta, pj, muj)
    eta_phi = atanh(max(min(phi,0.999),-0.999));
    eta_sh  = log(max(sigmah, 1e-6));
    eta_pj  = log(max(pj, 1e-6) / max(1 - pj, 1e-6));   % logit
    eta_muj = log(max(muj, 1e-6));
    theta = [alpha; beta; gamma(:); omega; eta_phi; eta_sh; delta(:); eta_pj; eta_muj];
end
...
function p = unpack_params(theta, K)
    ...
    idx = idx + 1; eta_pj = theta(idx);
    p.p_j = 1 / (1 + exp(-eta_pj));   % sigmoid
